...
3. Supabase: ask whether a Supabase project exists. If not, tell the user in plain steps: go to supabase.com → New project → name `gazelle` → choose a region near Virginia → wait for it to provision → Settings → API. Ask them to paste Project URL, anon key, and service role key. Write them to `.env.local`.
